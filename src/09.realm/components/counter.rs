...
/// MockComponent — переиспользуемая логика счётчика
#[derive(Default)]
struct Counter {
    props: Props,
    states: OwnStates,
...
// --- Application-level компоненты ---

/// Считает нажатия букв
    ...
    pub fn new(initial_value: isize) -> Self {
        Self {
            component: Counter::default()
                ...
                .value(initial_value)
                .label("Letter counter (press any letter)"),
        }
    }
}
    ...
    pub fn new(initial_value: isize) -> Self {
        Self {
            component: Counter::default()
                ...
                .modifiers(TextModifiers::BOLD)
                .value(initial_value)
                .label("Digit counter (press any digit)"),
        }
    }
}
    ...
    fn on(&mut self, ev: Event<NoUserEvent>) -> Option<Msg> {
        let cmd = match ev {
            ...
            Event::Keyboard(KeyEvent {
                code: Key::Tab,
                modifiers: KeyModifiers::NONE,
            }) => return Some(Msg::DigitCounterBlur),
            Event::Keyboard(KeyEvent {
                code: Key::Esc,
                modifiers: KeyModifiers::NONE,
            }) => return Some(Msg::AppClose),
